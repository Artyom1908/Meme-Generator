...
function MemeGenerator() {
    ...
    const [bottonMemeText, setBottonMemeText] = useState('Botton Text')
    const [memeUrl, setMemeUrl] = useState('')
    function handleMemeSelect(e) {
        setSelectedMeme(e.target.value)
    }

    function handleTopMemeText(e) {
        setTopMemeText(e.target.value)
    }

    function handleBottonMemeText(e) {
        setBottonMemeText(e.target.value)
    }

    function generateMeme(e) {
        e.preventDefault()
        const url = `https://apimeme.com/meme?meme=${selectedMeme}&top=${topMemeText}&bottom=${bottonMemeText}`
        setMemeUrl(url)
    }

    return (
        <div className="mameGenerator">
            <h2>Meme Generator</h2>
            <form onSubmit={generateMeme}>
                <label>
                    Select Meme Type:
                    <select value={selectedMeme} onChange={handleMemeSelect}>
                ...
                <label>
                    ...
                    <input
                        type="text"
                        value={bottonMemeText}
                        onChange={handleBottonMemeText}
                    />
                </label>
                <br />
